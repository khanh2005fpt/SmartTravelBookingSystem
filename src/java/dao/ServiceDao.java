...
// UPDATE - Cập nhật thông tin lịch trình chuyến bay (không thay đổi flightId)
    ...
    public Island getIslandById(int islandId) throws SQLException {
        String sql = "SELECT i.islandId, i.islandName, i.countryId, i.shortDescription, i.longDescription, i.bestSeason, i.activities, i.imageUrl, i.location, i.approvalStatus, c.countryName "
                + "FROM Islands i "
                + "LEFT JOIN Countries c ON i.countryId = c.countryId "
                + "WHERE i.islandId = ?";
        try {
